...
export function equalAmountSplits(
    people: Person[],
    total: number,
): SplitEntry[] {
    if (people.length === 0) return [];
...
export function validateSplits(
    ...
    if (mode === "percent") {
        ...
        if (Math.abs(sum - 100) > 0.01)
            return `Percentages sum to ${sum.toFixed(2)}%, must be 100%.`;
    } else {
...
export function buildPreview(
    ...
    return rows
        .map((row) => {
            ...
            const datetime =
                datetimeCol !== null ? parseDate(row[datetimeCol] ?? "") : "";
...
export function computeSettlements(
    ...
    while (ci < credits.length && di < debts.length) {
        ...
        const debt = debts[di]!;
        const amount =
            Math.round(Math.min(credit.amount, debt.amount) * 100) / 100;
...
export function validateAppState(parsed: unknown): AppState {
    ...
    const people: Person[] = (obj["people"] as unknown[]).map((p, i) => {
        ...
        if (typeof person["id"] !== "string")
            throw new Error(`people[${i}].id is not a string`);
        if (typeof person["name"] !== "string")
            throw new Error(`people[${i}].name is not a string`);
        return { id: person["id"], name: person["name"] };
    });

    const transactions: Transaction[] = (obj["transactions"] as unknown[]).map(
        (t, i) => {
            if (typeof t !== "object" || t === null)
                throw new Error(`transactions[${i}] is not an object`);
            const tx = t as Record<string, unknown>;
            if (typeof tx["id"] !== "string")
                throw new Error(`transactions[${i}].id is not a string`);
            ...
            if (!Array.isArray(tx["splits"]))
                throw new Error(`transactions[${i}].splits is not an array`);

            const splits: SplitEntry[] = (tx["splits"] as unknown[]).map(
                (s, j) => {
                    if (typeof s !== "object" || s === null)
                        throw new Error(
                            `transactions[${i}].splits[${j}] is not an object`,
                        );
                    const split = s as Record<string, unknown>;
                    if (typeof split["personId"] !== "string")
                        throw new Error(
                            `transactions[${i}].splits[${j}].personId is not a string`,
                        );
                    if (
                        split["mode"] !== "percent" &&
                        split["mode"] !== "amount"
                        ...
                        throw new Error(
                            `transactions[${i}].splits[${j}].mode is invalid`,
                        );
                    if (typeof split["value"] !== "number")
                        throw new Error(
                            `transactions[${i}].splits[${j}].value is not a number`,
                        );
                    return {
